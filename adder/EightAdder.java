...
import decimalToBinary.Converter8;
import twoCompliment.Compliment;

public class EightAdder {
	...
	public EightAdder(int one, int two) {
	...
	private void convertCompliment() {
		...
		if (decimal1 < 0) {
			...
			Converter8 converter = new Converter8();
	...
	public int checkOverFlow() {
		if ((cOut[0] + cOut[1]) == 1 || decimal1 > 127 || decimal1 < -127 || decimal2 < -127 || decimal2 > 127) {
			return 1;
		}
